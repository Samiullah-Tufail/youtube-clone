...
const CategoryVideos = () => {
  ...
  const getdata = () => {
    setArticle([]);
    setLoading("block");
    const url = `https://youtube-v3-lite.p.rapidapi.com/search?q=${category.category}&part=id%2Csnippet`;
    const options = {
      method: "GET",
      headers: {
  ...
  return (
    <div className="flex">
      <SideBar />
      <div className="flex flex-wrap w-full sm:ml-52 mt-32 sm:mt-20 lg:justify-center">
        <img
        ...
        {article.map((element) => (
          <div
            ...
            value={element.id}
            className=" text-white w-full md:card-width lg:card-width-27 m-4 "
          >
